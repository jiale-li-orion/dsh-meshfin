...
Drivers that can continue from an already committed replacement implement the narrow `ContextRunAgent` capability. The request is durable Session vocabulary rather than an inbox message, so no synthetic user text enters the next request.

```ts type-equiv
/** Durable request to run the next turn from a previously committed surface replacement. */
interface ContextRunRequest {
...
```ts type-equiv
/**
 * Narrow capability implemented by drivers that can start a turn from an
 * already committed context generation without another user message.
 */
interface ContextRunAgent extends Agent {
  /**
   * Request a turn against an already committed replacement generation. The
   * caller supplies the durable identity so its rewrite record can correlate
   * with this lifecycle before admission.
   * @param request - context generation and caller-minted lifecycle identity.
   * @throws when another context run is pending, the Agent is running, or the
   * request does not name the current replacement generation.
   */
  runFromContext(request: ContextRunRequest): void
}
```

The loop records `agent/context-run/requested`, then either `claimed` inside the owning turn or `cancelled` when cancellation or a newer generation makes it stale. An unclaimed request restores after restart and runs before later queued turns.
...
interface CancelOptions {
   ...
   * active turn is still aborted, but un-started message and context-run work
   * survives for a later turn and no cancellation record is logged.
   */
...
Source: [`packages/core/agent/src/runtime-types.ts:306`](../../packages/core/agent/src/runtime-types.ts)
...
Source: [`packages/core/agent/src/runtime-types.ts:213`](../../packages/core/agent/src/runtime-types.ts)
...
Source: [`packages/core/agent/src/runtime-types.ts:221`](../../packages/core/agent/src/runtime-types.ts)
...
Source: [`packages/core/agent/src/runtime-types.ts:202`](../../packages/core/agent/src/runtime-types.ts)
...
Source: [`packages/core/agent/src/runtime-types.ts:276`](../../packages/core/agent/src/runtime-types.ts)
...
Source: [`packages/core/agent/src/runtime-types.ts:194`](../../packages/core/agent/src/runtime-types.ts)
...
Source: [`packages/core/agent/src/runtime-types.ts:294`](../../packages/core/agent/src/runtime-types.ts)
