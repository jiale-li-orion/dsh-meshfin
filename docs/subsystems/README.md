...
| [session-context.md](session-context.md) | current Context units and generations, reviewed range preparation, same-Session rewrite, and bounded checkpoint recall |
...
| [workbench.md](workbench.md) | the shared workbench view, its `workbench/changed` event, and the session-fenced directory listing |
| [plugin-catalog.md](plugin-catalog.md) | the plugin catalog capability: the published index provider and the model-facing search/install tools |
