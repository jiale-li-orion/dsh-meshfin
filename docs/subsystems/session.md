...
interface SessionSurface {
  ...
  /** Seq of the replacement that created the current generation, or null before the first replacement. */
...
interface SurfaceFoldResult {
  ...
  /** Seq of the replacement that created the current surface generation, or null for the initial generation. */
...
`Session.readLog()` captures one immutable logical prefix without expanding the complete log. Point reads, bounded forward and reverse iterators, event-type indexes, and chunk-run summaries remain allocation-free at the array level; `materialize()` is the explicit operation for consumers that require a frozen array. A later append creates a new cut and never changes an earlier cut's length or logical events.
...
### `SessionSurfaceCut` — stable Context membership and its log prefix

`Session.readSurface()` captures the current model-visible membership together with the exact immutable log prefix that contains it. Capturing copies only the surface seq list, and repeated reads reuse one cut until append.
...
`Session` keeps physical storage private. Eligible consecutive `assistant/chunk` deltas share packed records, while every public read still observes immutable logical `SessionEvent`s with the original contiguous seq and timestamps. A compact per-type span index lets selected-type readers skip unrelated packed rows. `session.events` remains the compatibility face for a complete frozen array and therefore performs explicit full materialization on first access after an append.

Persistence restores parser-owned records through `fromRestoreRecordsAsync()`: packed rows remain packed, accepted parser slots are released as ownership transfers, and the unpublished restore yields between bounded CPU intervals. `fromRestoreRecords()` retains synchronous compatibility for callers that already own their scheduling.

`SessionEventFold<State>` is the shared live-fold helper for plugin state derived from a small event set. It initializes lazily, advances directly from committed `session/event` notifications, replays only a missed selected-type prefix, and moves its watermark only after the reducer returns successfully. Reducers must leave mutable state retryable when they throw.
...
What a persistence backend relies on: the durable log persists every event losslessly, **including** `assistant/chunk` — `seq` must stay contiguous, so chunks cannot be filtered out of the canonical log. A backend may choose its own storage encoding for an event batch as long as logical reads reconstruct the exact appended events (the JSONL backend's default packed chunk rows are such an encoding — see [persistence.md](persistence.md)). All `event.data` must be JSON-serializable; `Session.append` enforces this at the source, so a bad event never enters the log and `session.readLog()` always exposes exactly what a backend can persist. `session.events` is the explicit complete-array materialization of that same logical log. Adding an event type that carries non-serializable data, corrupts core execution nesting, or violates its owner's declared relation is a breaking change to the on-disk format.
...
Source: [`packages/core/session/src/index.ts:1405`](../../packages/core/session/src/index.ts)
...
Source: [`packages/core/session/src/index.ts:110`](../../packages/core/session/src/index.ts)
