...
`Session.readLog()` 无需展开完整日志即可捕获一个不可变的逻辑前缀。点读取、有界正向／反向迭代器、事件类型索引和分片连续段摘要都不会在数组层面分配内存；只有确实需要冻结数组的消费方才显式调用 `materialize()`。后续 append 会创建新切片，既不改变早先切片的长度，也不改变其中的逻辑事件。
...
### `SessionSurfaceCut` — 稳定 Context 成员关系及其日志前缀

`Session.readSurface()` 会捕获当前模型可见成员关系，以及包含这些成员的准确不可变日志前缀。捕获只复制表层 seq 列表；下一次追加前，重复读取会复用同一 cut。
...
`Session` 将物理存储保持为私有实现。符合条件的连续 `assistant/chunk` delta 共用打包记录，但所有公共读取仍会看到带原始连续 seq 与时间戳的不可变逻辑 `SessionEvent`。紧凑的逐类型连续区间索引使按类型读取的消费方能够跳过无关打包行。`session.events` 继续作为完整冻结数组的兼容接口，因此每次 append 后首次访问它时都会显式完整实体化。

持久化通过 `fromRestoreRecordsAsync()` 恢复由解析器独占的记录：打包行继续保持打包状态；所有权移交后立即释放已接纳的解析器槽位；未发布恢复过程在有界 CPU 区间之间让出执行权。已经自行管理调度的调用方仍可使用同步兼容接口 `fromRestoreRecords()`。

`SessionEventFold<State>` 是插件根据少量事件类型派生实时状态时使用的共享折叠辅助类。它惰性初始化，直接从已提交的 `session/event` 通知前进，只回放遗漏的选定类型前缀，并且只在 reducer 成功返回后推进水位线。reducer 抛出异常时，必须让可变状态保持可重试。
...
持久化后端依赖的约定如下：持久日志无损保存每个事件，**包括** `assistant/chunk`；`seq` 必须连续，因此不能从规范日志中过滤分片。后端可以为事件批次选择自己的存储编码，只要逻辑读取能重建与追加时完全一致的事件即可（JSONL 后端默认启用的打包分片行就是此类编码；见 [persistence.md](persistence.md)）。所有 `event.data` 都必须可序列化为 JSON；`Session.append` 会从源头强制这一要求，因此错误事件绝不会进入日志，`session.readLog()` 始终暴露与后端可持久化内容完全一致的逻辑日志。`session.events` 是同一逻辑日志显式实体化出的完整数组。新增会携带不可序列化数据、破坏核心执行嵌套或违反事件所有方声明关系的事件类型，都会构成磁盘格式的破坏性变更。
