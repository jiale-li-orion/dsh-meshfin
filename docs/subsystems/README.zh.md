...
| [session-context.md](session-context.md) | 当前 Context 单元与代际、可评审范围 preparation、同 Session 改写和有界检查点召回 |
...
| [browser-use.md](browser-use.md) | browser use seam：`ctx.browserUse`、独占 provider 名额、Playwright MCP provider 与启动/附着之分 |
| [computer-use.md](computer-use.md) | computer use seam：`ctx.computerUse`、独占 provider 名额、Cua Driver provider 与截图前提 |
...
| [workbench.md](workbench.md) | 共享工作台视图、其 `workbench/changed` 事件，以及以会话为围栏的目录列举 |
| [plugin-catalog.md](plugin-catalog.md) | 插件目录能力：已发布索引的 provider，以及面向模型的搜索/安装工具 |
