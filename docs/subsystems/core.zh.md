...
能够从已提交替换继续运行的驱动会实现收窄的 `ContextRunAgent` 能力。该请求属于持久 Session 词汇，而不是 inbox 消息，因此下一次请求不会加入合成用户文字。
...
agent loop 会记录 `agent/context-run/requested`，然后在所属轮次中记录 `claimed`，或在取消与较新代际使其陈旧时记录 `cancelled`。尚未认领的请求会在重启后恢复，并先于之后排队的轮次运行。
