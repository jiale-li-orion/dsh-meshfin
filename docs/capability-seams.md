...
| `ctx.compactions` | `seam` | [`compaction`](../packages/compaction/compaction) | [`compaction-basic`](../packages/compaction/compaction-basic) | [`session-context`](../packages/session/session-context) | - | Providers register for one Agent composition or as the process fallback; Session Context resolves the exact provider selected for the target Agent. |
| `ctx.sessionContext` | `core` | [`session-context`](../packages/session/session-context) | - | [`tool-session-context`](../packages/session/tool-session-context) | - | Owns bounded current-context inspection, append-only same-Session replacement, reviewed range compaction, and checkpoint-original recall. |
...
| `ctx.workbench` | `core` | [`workbench`](../packages/workbench/workbench) | - | - | - | One process-global view both the browser and agent tools mutate; every commit emits workbench/changed, and directory listings are fenced to the calling session workspace. |
| `ctx.pluginCatalog` | `seam` | [`plugin-catalog`](../packages/workbench/plugin-catalog) | [`plugin-catalog-awesome`](../packages/workbench/plugin-catalog-awesome) | `tool-plugin-catalog` | - | The Definition owns the entry and query vocabulary; the provider owns index transport, validation, and caching, and the tools resolve an install target only through an entry a search returned. |
| `ctx.pluginInstall` | `core` | [`plugin-install`](../packages/workbench/plugin-install) | - | `tool-plugin-catalog` | - | One install implementation serves both planes: the agent tool gates the call behind ctx.approval, the marketplace panel is the human gesture, and both hand the service a catalog URL rather than a command. |
