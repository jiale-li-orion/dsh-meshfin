...
flowchart LR
  ...
  svc_compactions["ctx.compactions<br/>Agent-scoped compaction provider directory"]
  pkg_session_context["session-context"]
  svc_sessionContext["ctx.sessionContext<br/>Current model-context management"]
  ...
  svc_workbench["ctx.workbench<br/>Shared workbench view state"]
  pkg_plugin_catalog["plugin-catalog"]
  svc_pluginCatalog["ctx.pluginCatalog<br/>Published plugin catalog"]
  pkg_plugin_catalog_awesome["plugin-catalog-awesome"]
  pkg_tool_plugin_catalog["tool-plugin-catalog"]
  pkg_plugin_install["plugin-install"]
  svc_pluginInstall["ctx.pluginInstall<br/>Approval-gated plugin install"]
  pkg_e2b["e2b"]
  ...
  pkg_compaction --> svc_compactions
  ...
  pkg_plugin_catalog --> svc_pluginCatalog
  pkg_plugin_catalog_awesome --> svc_pluginCatalog
  pkg_plugin_install --> svc_pluginInstall
  pkg_pwsh_local --> svc_shell
  pkg_sandbox --> svc_sandbox
  pkg_sandbox_local --> svc_sandbox
  pkg_sandbox_policy --> svc_sandboxPolicy
  pkg_session --> svc_sessions
  pkg_session_context --> svc_sessionContext
  ...
  svc_pluginCatalog --> pkg_tool_plugin_catalog
  svc_pluginInstall --> pkg_tool_plugin_catalog
  svc_sandbox --> pkg_bash_sandbox
  svc_sandbox --> pkg_terminal_bash
  svc_sandboxPolicy --> pkg_bash_sandbox
  svc_sandboxPolicy --> pkg_fs_sandbox
  svc_sandboxPolicy --> pkg_terminal_bash
  svc_sessionContext --> pkg_tool_session_context
...
| `ctx.compactions` | `seam` | [`compaction`](../packages/compaction/compaction) | [`compaction-basic`](../packages/compaction/compaction-basic) | [`session-context`](../packages/session/session-context) | - | 提供方按一个 Agent 组合注册，或作为进程 fallback；Session Context 为目标 Agent 解析确切选中的提供方。 |
| `ctx.sessionContext` | `core` | [`session-context`](../packages/session/session-context) | - | [`tool-session-context`](../packages/session/tool-session-context) | - | 拥有有界的当前 Context 检查、仅追加的同 Session 替换、可评审范围压缩和检查点原文召回。 |
...
| `ctx.workbench` | `core` | [`workbench`](../packages/workbench/workbench) | - | - | - | 浏览器与 agent 工具共同修改的进程全局视图；每次提交都发出 workbench/changed，目录列举以调用会话的工作区为围栏。 |
| `ctx.pluginCatalog` | `seam` | [`plugin-catalog`](../packages/workbench/plugin-catalog) | [`plugin-catalog-awesome`](../packages/workbench/plugin-catalog-awesome) | `tool-plugin-catalog` | - | Definition 持有条目与查询词表；provider 负责索引传输、校验与缓存；工具只通过某次搜索返回的条目解析安装目标。 |
| `ctx.pluginInstall` | `core` | [`plugin-install`](../packages/workbench/plugin-install) | - | `tool-plugin-catalog` | - | 一份安装实现服务两个平面：agent 工具把调用挡在 ctx.approval 之后，市场面板是人的手势，两者都只把目录 URL 交给服务而不是命令。 |
...
| `ctx.browserUse` | `seam` | [`browser-use`](../packages/browser-use/browser-use) | [`browser-use-playwright-mcp`](../packages/browser-use/browser-use-playwright-mcp) | [`browser-use-playwright-mcp`](../packages/browser-use/browser-use-playwright-mcp) | - | 每个组合一个提供方占用该名额，直到它的浏览器工作结算完毕，因此释放后的注册永远不会与仍在关闭的浏览器重叠。 |
| `ctx.computerUse` | `seam` | [`computer-use`](../packages/computer-use/computer-use) | [`computer-use-cua-driver`](../packages/computer-use/computer-use-cua-driver) | [`computer-use-cua-driver`](../packages/computer-use/computer-use-cua-driver) | - | 每个组合一个提供方占用该名额，直到它的资源关闭完毕，因此释放后的注册永远不会与仍在关闭的桌面会话重叠。 |
