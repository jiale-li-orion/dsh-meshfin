...
[CI](../../../../.github/workflows/ci.yml) gives push and pull-request events complementary responsibilities. Pull requests run consolidated Linux and Wine-hosted Windows jobs plus the Node compatibility and Python contracts on standard GitHub-hosted capacity; an independent native Windows job reports the complete Windows inventory without participating in the required aggregate. This deployment carries no in-house runner pool, so there is no push-time standby lane and the failover machinery those lanes re-proved is gone ([hosted-runner decision](2026-09-19-hosted-runner-ci.md)). The standard-hosted `serial / macos` reference remains enabled, `serial / linux` stays disabled under `TODO(hosted-serial-ci)` until its portable capacity can be restored, and `serial / windows` went with the pool it stood by for. The separate job definitions intentionally keep their short checkout, runtime setup, and immutable install sequences visible instead of hiding operating systems behind a matrix or reusable workflow. `workflow_dispatch` is reserved for runner benchmarks.
...
The portable reference uses GitHub's standard `ubuntu-latest`, `macos-latest`, and `windows-2025` labels. The required Windows job runs under Wine on `ubuntu-latest`, while the independent native job runs on `windows-2025` and is absent from the required aggregate under the [dual Windows decision](2026-08-08-native-windows-pull-request-ci.md); `serial / windows` went with the in-house pool it stood by for. Required jobs use portable standard capacity under the [required-CI decision](2026-07-23-portable-required-pull-request-ci.md). The runner benchmarks that compared larger pools now run one lane per platform on the hosted labels, because a correctness path must remain runnable without repository-external runner configuration.
