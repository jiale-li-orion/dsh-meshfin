...
本部署改为全部泳道跑在 GitHub 标准托管运行器上，因为无论是组织级池还是公司自有池对它都不可用；上文所述的切换机制以及用于验证它的热备通道均已移除（[托管运行器决策](2026-09-19-hosted-runner-ci.md)）。仍然沿用下来的是证据标准：容量变更的依据是实测的泳道耗时，而不是核心数量；用于比较各池的基准矩阵现在每个平台只跑一条泳道，并使用托管标签。
