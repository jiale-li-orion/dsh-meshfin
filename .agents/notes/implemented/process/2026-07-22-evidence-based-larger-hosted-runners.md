...
This deployment runs every lane on GitHub's standard hosted runners instead, because neither the organization-level pools nor the in-house pool are available to it; the switch described above and the standby lane that proved it are gone ([hosted-runner decision](2026-09-19-hosted-runner-ci.md)). What still carries over is the evidence standard: a capacity change is justified by measured lane timing rather than by a plane count, and the benchmark matrices that compared pools now run one lane per platform on the hosted labels.
