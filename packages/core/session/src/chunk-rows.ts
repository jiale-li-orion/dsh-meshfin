...
/**
 * Whether one trusted storage record is a packed chunk row.
 * @param record - storage value to classify.
 * @returns whether the value carries one packed row tag.
 */
export function isChunkRow(record: unknown): record is ChunkRow {
  return isRecord(record) && (record.type === 'text-chunks'
...
/**
 * Number of logical session events represented by one trusted chunk row.
 * @param row - packed chunk row.
 * @returns the number of logical members.
 */
...
/**
 * Last logical member timestamp without allocating a per-member time cache.
 * @param row - packed chunk row.
 * @returns the final member's epoch-millisecond timestamp.
 */
export function chunkRowLastTime(row: ChunkRow): number {
  let time = row.time0
  for (const gap of row.data.dt) time += gap
  return time
}

/** Build one member event after the caller resolves its exact timestamp. */
function rowMember(row: ChunkRow, offset: number, time: number): SessionEvent<'assistant/chunk'> {
  const members = row.type === 'tool-call-chunks' ? row.data.args : row.data.texts
  let chunk: StreamChunk
  switch (row.type) {
    case 'text-chunks':
      chunk = { type: 'text-delta', index: row.data.index, text: members[offset] as string }
      break
    case 'reasoning-chunks':
      chunk = { type: 'reasoning-delta', index: row.data.index, text: members[offset] as string }
      break
    case 'tool-call-chunks':
      chunk = {
        ...
        argumentsDelta: members[offset] as string,
      }
      break
    /* v8 ignore next 2 -- ChunkRow is a closed union. */
    default:
      return assertNever(row, 'chunk-rows rowMember')
  }
  return {
...
/** Lazily cached absolute member times for repeated point reads of one row. */
const rowTimes = new WeakMap<ChunkRow, number[]>()

/** Resolve one member timestamp, extending only this row's point-read cache. */
function rowTimeAt(row: ChunkRow, offset: number): number {
  let times = rowTimes.get(row)
  if (times === undefined) {
    times = [row.time0]
    rowTimes.set(row, times)
  }
  while (times.length <= offset) {
    const gap = row.data.dt[times.length - 1] as number
    times.push((times[times.length - 1] as number) + gap)
  }
  return times[offset] as number
}

/**
 * Decode one member of a trusted chunk row without expanding its siblings.
 * @param row - validated or encoder-produced packed row.
 * @param offset - zero-based member offset.
 * @returns the reconstructed event.
 */
export function chunkRowAt(row: ChunkRow, offset: number): SessionEvent<'assistant/chunk'> {
  if (!Number.isSafeInteger(offset) || offset < 0 || offset >= chunkRowLength(row)) {
    throw new RangeError(`chunk row offset ${String(offset)} is outside [0, ${String(chunkRowLength(row))})`)
  }
  return rowMember(row, offset, rowTimeAt(row, offset))
}

/**
 * Decode a half-open member range in ascending order without retaining an
 * expanded event array or an absolute timestamp per member.
 * @param row - validated or encoder-produced packed row.
 * @param from - inclusive member offset.
 * @param to - exclusive member offset.
 * @returns reconstructed events in logical sequence order.
 */
export function* chunkRowValues(
  row: ChunkRow,
  from = 0,
...
/**
 * Decode a half-open member range in descending order without materializing it.
 * @param row - validated or encoder-produced packed row.
 * @param from - inclusive lower member offset.
 * @param to - exclusive upper member offset.
 * @returns reconstructed events in reverse logical sequence order.
 */
export function* reverseChunkRowValues(
  row: ChunkRow,
  from = 0,
  to = chunkRowLength(row),
): Generator<SessionEvent<'assistant/chunk'>> {
...
/**
 * Extend an encoder-produced tail row with the next exact delta event.
 * @param row - mutable private tail row.
 * @param previous - previous logical event in the row.
 * @param next - candidate event at the next seq.
 * @returns whether `next` was absorbed into `row`.
 */
export function extendChunkRow(row: ChunkRow, previous: SessionEvent, next: SessionEvent): boolean {
  const kind = classify(next)
  const expectedKind: DeltaKind = row.type === 'text-chunks'
    ? 'text-delta'
    : row.type === 'reasoning-chunks'
      ? 'reasoning-delta'
      : 'tool-call-delta'
  if (kind !== expectedKind || classify(previous) !== expectedKind) return false
  const previousDelta = previous as DeltaEvent
  const nextDelta = next as DeltaEvent
  if (!continues(previousDelta, nextDelta, kind)) return false
  const oldLength = chunkRowLength(row)
  row.data.dt.push(next.time - previous.time)
  if (row.type === 'tool-call-chunks') {
    row.data.args.push((nextDelta.data.chunk as { argumentsDelta: string }).argumentsDelta)
  } else {
    row.data.texts.push((nextDelta.data.chunk as { text: string }).text)
  }
  const times = rowTimes.get(row)
  if (times?.length === oldLength) times.push(next.time)
  return true
}

/** Append one array without using an argument spread that can exceed V8's call limit. */
function appendValues<T>(target: T[], source: readonly T[]): void {
  for (const value of source) target.push(value)
}

/**
 * Merge one exclusively owned storage row into a compatible mutable predecessor.
 * The operation preserves every logical member and token boundary while
 * removing physical write-batch boundaries from Session's private layout.
 * @param target - mutable predecessor retained by the private log.
 * @param targetLastTime - exact timestamp of `target`'s final logical member.
 * @param next - next validated row in logical sequence order.
 * @returns whether `next` was absorbed into `target`.
 */
export function mergeChunkRows(target: ChunkRow, targetLastTime: number, next: ChunkRow): boolean {
  if (target.type !== next.type
    || target.seq0 + chunkRowLength(target) !== next.seq0
    || target.data.turn !== next.data.turn
    || target.data.step !== next.data.step
    || target.data.index !== next.data.index) return false
  if (Object.isFrozen(target) || Object.isFrozen(target.data)) return false

  const gap = next.time0 - targetLastTime
  if (!Number.isSafeInteger(gap)) return false
  if (target.type === 'tool-call-chunks' && next.type === 'tool-call-chunks') {
    if (target.data.id !== next.data.id
      || Object.hasOwn(target.data, 'name') !== Object.hasOwn(next.data, 'name')
      || target.data.name !== next.data.name) return false
    target.data.dt.push(gap)
    appendValues(target.data.dt, next.data.dt)
    appendValues(target.data.args, next.data.args)
  } else if (target.type !== 'tool-call-chunks' && next.type !== 'tool-call-chunks') {
    target.data.dt.push(gap)
    appendValues(target.data.dt, next.data.dt)
    appendValues(target.data.texts, next.data.texts)
  } else {
    return false
  }
  rowTimes.delete(target)
  return true
}
...
/**
 * Logical starting sequence of one trusted storage record.
 * @param record - event or packed row.
 * @returns the first logical seq represented by the record.
 */
export function storageRecordStart(record: StorageRecord): number {
  return isChunkRow(record) ? record.seq0 : record.seq
}

/**
 * Logical event count represented by one trusted storage record.
 * @param record - event or packed row.
 * @returns the represented logical event count.
 */
export function storageRecordLength(record: StorageRecord): number {
  return isChunkRow(record) ? chunkRowLength(record) : 1
}

/**
 * Iterate the logical events represented by one trusted storage record.
 * @param record - event or packed row.
 * @returns the represented logical events in seq order.
 */
export function storageRecordValues(record: StorageRecord): Iterable<SessionEvent> {
  return isChunkRow(record) ? chunkRowValues(record) : [record]
}

/**
 * Iterate the logical events represented by trusted storage records.
 * @param records - storage records in logical order.
 * @returns all represented logical events in seq order.
 */
export function* storageRecordsValues(records: Iterable<StorageRecord>): Generator<SessionEvent> {
  for (const record of records) yield* storageRecordValues(record)
}

/**
 * Adopt one parsed JSONL value as a storage record without expanding a packed
 * chunk row. Row-tagged values validate before they cross the storage parser;
 * every other value remains an event candidate for Session validation.
 * @param value - one line's `JSON.parse` result.
 * @returns the event candidate or validated packed row.
 */
export function adoptStorageRecord(value: unknown): StorageRecord {
  if (!isRecord(value)) return value as SessionEvent
  const tag = value.type
  if (tag !== 'text-chunks' && tag !== 'reasoning-chunks' && tag !== 'tool-call-chunks') {
    return value as SessionEvent
  }
  return validateRow(value, tag)
}
...
export function decodeStorageRecord(value: unknown): SessionEvent[] {
  return [...storageRecordValues(adoptStorageRecord(value))]
}
