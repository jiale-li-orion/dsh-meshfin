...
`ContextRunAgent` is a narrow driver capability for starting a turn from an already committed replacement generation without another user message. `runFromContext(request)` requires an idle driver, the current `generationSeq`, and no pending context run. Its requested, claimed, and cancelled states are durable `agent/context-run/*` Session events; an unclaimed request restores after restart.
...
- Context continuation: drivers that support same-Session history replacement implement `ContextRunAgent`; ordinary `Agent` implementations are not required to expose zero-message continuation.
...
A capable driver sends the ordinary assembled request from an already committed replacement generation. The context-run lifecycle contributes no user text and no extra prompt section.

#### Token effect

Zero synthetic tokens. The request pays for the current surface and ordinary request envelope exactly as any other step does.

#### KV Cache effect

The replacement producer determines invalidation. The context run itself preserves every still-reusable prefix and appends later model output normally.
