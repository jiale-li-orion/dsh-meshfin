...
`ContextRunAgent` 是一项收窄的驱动能力，用于从已提交替换代际启动一个不含新用户消息的轮次。`runFromContext(request)` 要求驱动空闲、`generationSeq` 指向当前代际，并且没有待处理 context run。requested、claimed 与 cancelled 状态都是持久的 `agent/context-run/*` Session 事件；尚未认领的请求会在重启后恢复。
...
- 上下文继续：支持同 Session 历史替换的驱动实现 `ContextRunAgent`；普通 `Agent` 实现不需要暴露零消息继续能力。
...
具备该能力的驱动会从已提交替换代际发送普通组装请求。Context-run 生命周期不贡献用户文字或额外提示词章节。

#### Token 影响

合成 token 为零。该请求与普通步骤一样，只支付当前表层与普通请求 envelope 成本。

#### KV Cache 影响

替换生产方决定失效位置。Context run 本身会保留仍可复用的全部前缀，之后模型输出正常追加。
...
- **`cancel()` 默认清空待处理工作**：它会中止正在处理的轮次，并取消排队、steering 和尚未认领的 context-run 工作；`cancel(cause, { keepInbox: true })` 只中止轮次并保留待处理项。仍不存在只中止步骤、同时让正在处理的轮次继续运行的操作（[停止 API Agent Note](../../../.agents/notes/implemented/simplification/2026-06-20-public-agent-stop-api.md)）。
