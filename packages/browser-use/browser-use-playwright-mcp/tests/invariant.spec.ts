...
describe('browser-use-playwright-mcp invariant companion', () => {
  it('declares its identity and explained empty installer', async () => {
    expect(companion.name).toBe('browser-use-playwright-mcp-invariant')
    ...
    expect(registered).toEqual(['@deepseek-ai/dsh-browser-use-playwright-mcp'])
    dispose()
  })
})
