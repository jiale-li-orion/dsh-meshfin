...
function precedingMessageTime(agent: Agent): number | undefined {
  for (const event of agent.session.readLog().reverseValuesOf([
    'user/message',
    'assistant/message',
    'tool/result',
  ])) {
...
function precedingStepContextTime(agent: Agent, turn: number): number | undefined {
  for (const event of agent.session.readLog().reverseValuesOf(['turn/start', 'user/message'])) {
...
function latestInjectionTime(agent: Agent): number | undefined {
  for (const event of agent.session.readLog().reverseValuesOf(['user/message'])) {
